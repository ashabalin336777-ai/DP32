...
**Демо:** [https://dp32.shastudio.ru](https://dp32.shastudio.ru) — **выбор МК** по параметрам, **сравнение цены и наличия** (ЧипДип / Платан / Промэлектроника), **график** scatter. Цифры — Pandas, Chart.js только рисует.  
**Репозиторий:** [ashabalin336777-ai/DP32](https://github.com/ashabalin336777-ai/DP32)  
**Текст для презентации (оглавление, стек, фичи):** [docs/prezentaciya-diploma.md](docs/prezentaciya-diploma.md)
...
Слева — простыми словами, справа — как это называется в коде и в пояснительной записке.

| Простыми словами | Технически |
|------------------|------------|
| Язык всей программы | Python 3.11+ |
| Таблицы, проценты, «кто дешевле / у кого больше на складе» | Pandas |
| Поиск похожих МК по памяти, частоте, выводам | scikit-learn: `StandardScaler` + `NearestNeighbors` (k=5) |
| База в одном файле, без отдельного сервера БД | SQLite (WAL, `sqlite3`, без ORM / SQLAlchemy) |
| Быстро скачать страницы каталога | httpx + BeautifulSoup (CSS-селекторы) |
| Открыть карточку товара как в браузере | Playwright (Chromium) |
| Нейросеть заполняет бланк спецификации из HTML | Neural Deep Pro, Qwen2.5-14B, `instructor` + Pydantic |
| Нейросеть пишет выводы отчёта, числа не считает | Qwen2.5-32B + FactValidator (`VALID` / `INVALID`) |
| Найти ссылку на карточку по артикулу | Neural Deep `search:web` |
| Страницы сайта и PDF | Jinja2; PDF — WeasyPrint (на Windows запасной путь Playwright) |
| Пузырьки на графике | Chart.js (отрисовка); радиус считает Pandas |
| Сайт без FastAPI/Streamlit | `http.server` + Jinja2 (`src/web.py`) |
| Запуск на сервере | Docker Compose, Timeweb VPS, nginx SHA Studio |
| Ключи не в git | python-dotenv, `.env` |
| Не гонять одну и ту же страницу в нейросеть повторно | diskcache |
| Проверки без живого LLM | pytest |

## Возможности

| Простыми словами | Технически |
|------------------|------------|
| Отфильтровать МК по бренду, ядру, памяти, корпусу | `/finder`: фасеты + диапазоны, дедуп MPN, **по 50 строк** на страницу |
| Один артикул — цена и склад трёх магазинов | `/compare?part=`: матрица Pandas, Δ к min цене и max остатку |
| Карта «где дешевле и где лежит» | `/charts`: bubble scatter наличие × цена, обзор до 400 точек |
| Сама листает каталоги поставщиков | `fast_scraper` (httpx), семена `catalog_seeds.json`, upsert SQLite |
| Найти близкие аналоги своей линейки | pin-compatible фильтр + KNN, дельты `(our − comp) / comp`, порог 5% |
| PDF-отчёт, в котором нельзя соврать цифрой | агент 2 + FactValidator; статус `draft` (человек подтверждает) |
| Живёт в интернете | https://dp32.shastudio.ru, контейнер `mcu-analyzer-web` |

## Преимущества

| Простыми словами | Технически |
|------------------|------------|
| Нейросеть не выдумывает цены и проценты | Числа только Pandas / scikit-learn; LLM копирует поля из таблицы |
| Не нужен дорогой сервер баз данных | SQLite WAL, бэкап копированием файла |
| Один запрос вместо трёх ручных сайтов | Снимок `load_latest_snapshot` по ЧипДип / Платан / Промэлектроника |
| Сайт поставщика закрылся — остальное считается | 403/timeout: backoff, skip URL, пайплайн не падает |
| Секреты не светятся на GitHub | ключи только в `.env` |
| Можно защищать диплом на фикстурах | pytest без сети и без Neural Deep |
| Человек остаётся в контуре | Human-in-the-Loop, `needs_review` при `llm_confidence < 0.8` |
| Каталог на тысячи строк не «вешает» страницу | пагинация finder, потолок точек графика, httpx на листингах |
...
| Маршрут | Назначение |
|---------|------------|
| `/finder` | Фильтры по артикулу, номенклатурному №, бренду, ядру, flash, частоте, корпусу, температуре; таблица уникальных МК **по 50 на страницу** |
